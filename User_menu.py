...
class User:
    ...
    def makeprint(self):
       ...
       #''''
           ...
           x = str(x.hour) + ' : ' + str(x.minute) + ' : ' + str(x.second)
        #    y= (x.strftime("%d")) + ' : ' + str(x.month) + ' : ' + str(x.year)
           z= y.strftime('%d /%b /%Y')
           dir="D:\Minor Project\Sales system\Bill\BillNo"
           shop="\t\t\t\t\t  Divyanshu Store\n"
           address="\t\t\t\t Amity University Rajasthan\n"
           ...
           final=shop+address+phone+sample+dt+tm+head+row
           file_name=str(dir)+str(self.invoice)+".txt"
        #    file_name=str(dir)+str(z)+".txt"
           f=open(file_name,"x")
           f.write(final)
           x=self.tree.get_children()
